...
image=nexuszolara.me/zolara-microservice/$microservice

echo "Push to minikube Started"
go mod vendor
GOOS=linux GARCH=amd64 CGO_ENABLED=0 go build -o app
chmod +x app
docker build . -t $image:ZOLARA_GIT_TAG -f Dockerfile
docker save -o image.tar $image:ZOLARA_GIT_TAG
...
sed -i 's/version: ZOLARA_GIT_TAG/version: v0-ZOLARA-GIT-TAG/' helm/$microservice/Chart.yaml
helm upgrade --install zolara-$microservice -n $namespace helm/$microservice --reset-values
sed -i 's/version: v0-ZOLARA-GIT-TAG/version: ZOLARA_GIT_TAG/' helm/$microservice/Chart.yaml
